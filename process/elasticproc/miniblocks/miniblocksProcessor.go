...
func (mp *miniblocksProcessor) PrepareDBMiniblocks(headerData *data.HeaderData, miniBlocks []*block.MiniBlock) []*data.Miniblock {
	selfShard := headerData.ShardID
	...
	for mbIndex, miniBlock := range miniBlocks {
		...
		dbMiniBlock, errPrepareMiniBlock := mp.prepareMiniblockForDB(mbIndex, miniBlock, headerData)
...
func (mp *miniblocksProcessor) prepareMiniblockForDB(
	...
	encodedHeaderHash := hex.EncodeToString(headerData.HeaderHash)
	isIntraShard := dbMiniblock.SenderShardID == dbMiniblock.ReceiverShardID
	isCrossOnSource := !isIntraShard && dbMiniblock.SenderShardID == headerData.ShardID
	if isIntraShard || isCrossOnSource {
		mp.setFieldsMBIntraShardAndCrossFromMe(mbIndex, headerData, encodedHeaderHash, dbMiniblock, isIntraShard)

		return dbMiniblock, nil
	}

	processingType, _ := mp.computeProcessingTypeAndConstructionState(mbIndex, headerData)
...
func (mp *miniblocksProcessor) setFieldsMBIntraShardAndCrossFromMe(
	...
	processingType, constructionState := mp.computeProcessingTypeAndConstructionState(mbIndex, headerData)
...
func (mp *miniblocksProcessor) computeProcessingTypeAndConstructionState(mbIndex int, headerData *data.HeaderData) (string, int32) {

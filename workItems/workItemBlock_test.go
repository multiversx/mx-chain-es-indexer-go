...
func TestItemBlock_SaveHeaderShouldErr(t *testing.T) {
	...
	itemBlock := workItems.NewItemBlock(
		&mock.ElasticProcessorStub{
			SaveHeaderCalled: func(headerHash []byte, header data.HeaderHandler, signersIndexes []uint64, body *dataBlock.Body, notarizedHeadersHashes []string, gasConsumptionData indexer.HeaderGasConsumption, txsSize int) error {
				return localErr
			},
		},
		&mock.MarshalizerMock{},
